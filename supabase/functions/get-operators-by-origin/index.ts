...
console.log(`Function "get-operators-by-origin" up and running!`)

/**
 * This function finds all operators flying FROM a specific origin airport
...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Expect origin_code, start_time, end_time
    const { origin_code, start_time, end_time } = await req.json()

    // Basic validation
    if (!origin_code || !start_time || !end_time) {
      throw new Error("Missing required parameters: origin_code, start_time, end_time") // Updated parameter list
    }
    ...
    try {
      ...
      // Filters ONLY by origin_code. <--- CHANGE HERE
      ...
      const sql = `
        ...
        WHERE m.origin_code = $1 -- Filter ONLY by origin <--- CHANGE HERE
      ...
      console.log(`Found ${operatorData.length} operators flying from "${origin_code}" within the period.`);
    ...
    console.error("Error in get-operators-by-origin:", error.message, error.stack ? `\nStack: ${error.stack}` : '')
...
/* To invoke locally:
  ...
  curl -i --location --request POST 'http://127.0.0.1:54321/functions/v1/get-operators-by-origin' \
