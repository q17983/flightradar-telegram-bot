...
# --- Main Execution Block (Integrated Specific Type Handling) ---
if __name__ == "__main__":
    ...
    try:
        ...
        for type_index, target_aircraft_type in enumerate(AIRCRAFT_TYPES_TO_SCRAPE):
            ...
            logging.info(f"--- Fetching history for {len(aircraft_tuples_found)} registrations from {target_aircraft_type} list ---")
            for i, (reg_to_fetch, specific_type, airline_name) in enumerate(aircraft_tuples_found):
                ...
                logging.info(f"--- Processing History for {specific_type} ({target_aircraft_type} list) reg {i+1}/{len(aircraft_tuples_found)}: {reg_to_fetch} ---")
                ...
                if history_html:
                    ...
                    if "could not find data for specified flight" in history_html.lower():
                        ...
                        write_progress(target_aircraft_type, reg_to_fetch) # Record progress
                    else:
                        # Parse History
                        flight_records = extract_flight_history(history_html, reg_to_fetch, latest_ts)
                        if flight_records:
                            # Save *only potentially new* records
                            processed = save_flight_history_to_db(connection, flight_records)
                            if processed >= 0:
                                total_movements_processed_overall += len(flight_records)
                                logging.info(f"Attempted to save {len(flight_records)} potentially new movement records for {reg_to_fetch}.")
                                write_progress(target_aircraft_type, reg_to_fetch) # Record progress
                            else:
                                logging.warning(f"Database save function indicated an error for {reg_to_fetch}. Progress not saved.")
                        else:
                            logging.warning(f"No *new* flight records parsed for {reg_to_fetch}.")
                            write_progress(target_aircraft_type, reg_to_fetch) # Record progress
                else:
            ...
            logging.info(f"--- Finished processing history for aircraft from {target_aircraft_type} list ---")
